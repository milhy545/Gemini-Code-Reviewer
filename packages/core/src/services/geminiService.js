...
const getReviewFocusInstructions = (focus, language) => {
...
export const reviewCode = async (code, language, model = 'gemini-2.5-flash', reviewFocus = 'complete') => {
    const languageLabel = language.charAt(0).toUpperCase() + language.slice(1);
...
export const correctCode = async (code, language, model = 'gemini-2.5-flash', reviewFocus = 'complete') => {
    const languageLabel = language.charAt(0).toUpperCase() + language.slice(1);
    // Add focus-specific correction instructions
    const focusNote = reviewFocus !== 'complete'
        ? `Zaměřte se především na opravu problémů souvisejících s: ${reviewFocus === 'security' ? 'bezpečností' :
...
export const estimateApiCallInfo = (codeLength, model) => {
